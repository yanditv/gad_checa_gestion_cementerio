...
Para incluir el archivo de catastro en Docker:

1. **Renombrar archivo migrado (si existe)**:

   ```bash
   # Si ya tienes un archivo migrado, renómbralo a CATASTRO_FINAL.xlsx
   cp CATASTRO_FINAL_MIGRADO_*.xlsx CATASTRO_FINAL.xlsx
   ```
...
2. **Colocar el archivo en la raíz del proyecto**:
   ...
   ├── CATASTRO_FINAL.xlsx    <- Aquí debe estar el archivo
   ├── Dockerfile
   ├── Program.cs
   └── ...
   ```
...
3. **Construir la imagen Docker**:
   ...
   ```bash
   docker build -t gad-cementerio .
   ```

4. **El Dockerfile copiará automáticamente el archivo** si existe durante el build

5. **Si no quieres migrar el catastro**, simplemente no incluyas el archivo `CATASTRO_FINAL.xlsx` y la aplicación funcionará normalmente (mostrará el mensaje "Archivo CATASTRO_FINAL.xlsx no encontrado. Saltando migración del catastro.")
